...
class SimpleMinutiaeNetFeatureExtractor(nn.Module):
    """
    Simplified MinutiaeNet feature extractor that better matches the available weights.
    """
    
    def __init__(self, input_shape=(400, 400, 1), embedding_size=512, pretrained_path=None):
        super(SimpleMinutiaeNetFeatureExtractor, self).__init__()
        
        self.input_shape = input_shape
        self.embedding_size = embedding_size
        
        # Simplified architecture that better matches the available weights
        # Block 1 - matches conv2d (64, 1, 7, 7)
        self.conv1 = nn.Conv2d(1, 64, kernel_size=7, padding=3)
        self.bn1 = nn.BatchNorm2d(64)
        self.pool1 = nn.MaxPool2d(2)
        
        # Block 2 - matches conv2d_1, conv2d_2
        ...
        # Block 3 - matches conv2d_3, conv2d_4
    ...
    def forward(self, x):
        """
        Forward pass through simplified MinutiaeNet feature extractor.
...
class SimpleMinutiaeNetSiamese(nn.Module):
    """
    Siamese network using simplified MinutiaeNet as feature extractor.
    """
    
    def __init__(self, input_shape=(400, 400, 1), embedding_size=512, pretrained_path=None):
        super(SimpleMinutiaeNetSiamese, self).__init__()
        
        # Simplified MinutiaeNet feature extractor
        self.feature_extractor = SimpleMinutiaeNetFeatureExtractor(
...
def create_simple_minutiaenet_model(device, pretrained_path=None, **kwargs):
    """
    Factory function to create and initialize a SimpleMinutiaeNetSiamese model.
    ...
    Returns:
        Initialized SimpleMinutiaeNetSiamese on the specified device
    """
    model = SimpleMinutiaeNetSiamese(pretrained_path=pretrained_path, **kwargs).to(device)
    return model 